...
import uk.gov.hmrc.personaldetailsvalidation.model.{NinoDetails, NinoDetailsForm}
import uk.gov.hmrc.personaldetailsvalidation.utils.ComponentSpecHelper

class NinoDetailsFormISpec extends ComponentSpecHelper {

  "NinoDetailsForm" should {

    "bind a valid NINO in standard format" in {
    ...
    "bind a NINO with spaces and lowercase letters, normalising it" in {
    ...
    "return an error for a completely invalid NINO format" in {
      val bound = NinoDetailsForm.ninoForm.bind(Map("nino" -> "NOTANINO"))
    ...
    "return an error when the NINO field is empty" in {
      ...
      bound.errors should not be empty
      bound.errors.head.message shouldBe "personal-details.nino.required"
    }
    ...
    "unbind a NinoDetails back to a map" in {
      val details = NinoDetails(Nino("AA000003D"))
      val data    = NinoDetailsForm.ninoForm.fill(details).data

      data.get("nino") shouldBe Some("AA000003D")
    }
  }
}
