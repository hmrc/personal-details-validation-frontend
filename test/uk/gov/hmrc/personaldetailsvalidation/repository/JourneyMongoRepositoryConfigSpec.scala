...
class JourneyMongoRepositoryConfigSpec extends UnitSpec {

  "config" should {
    "return ttl duration from configuration" in new Setup {
      whenConfigEntriesExists("mongodb.collections.journey.ttl" -> "P1D") { config =>
    ...
    "throw DateTimeParseException if configured ttl is not a valid ISO date time syntax" in new Setup {
      whenConfigEntriesExists("mongodb.collections.journey.ttl" -> "P1 D") { config =>
  ...
  private trait Setup extends ConfigSetup[JourneyMongoRepositoryConfig] {
    val newConfigObject: Configuration => JourneyMongoRepositoryConfig = new JourneyMongoRepositoryConfig(_)
  }
