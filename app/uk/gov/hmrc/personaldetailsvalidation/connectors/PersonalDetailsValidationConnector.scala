...
trait PersonalDetailsValidationConnector[Interpretation[_]] {
...
@Singleton
class FuturedPersonalDetailsValidationConnector
  extends PersonalDetailsValidationConnector[Future] {

  override def passToValidation(personalDetails: PersonalDetails)
                               (implicit headerCarrier: HeaderCarrier,
                                executionContext: ExecutionContext): Future[String] =
    Future.successful("abc")
}