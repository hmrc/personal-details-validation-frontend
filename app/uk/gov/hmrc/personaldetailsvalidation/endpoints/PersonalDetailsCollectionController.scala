...
class PersonalDetailsCollectionController @Inject()(personalDetailsSubmission: FuturedPersonalDetailsSubmission,
  ...
  private def submitPersonalDetails(personalDetails: PersonalDetails, completionUrl: CompletionUrl, isLoggedInUser: Boolean)
                                   (implicit request: Request[_]): EitherT[Future, Result, Result] = {
    for {
